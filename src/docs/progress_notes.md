...
## 30日レンジ位置モジュールを追加(2026-07-21)

注目銘柄について、直近30営業日の値動きレンジ(高値・安値)の中で現在値がどの位置に
あるかを知りたい、という要望を受けて実装した。

- `stock_watchlist`の取得期間を15営業日→30営業日に拡張し、既存の10日分前日比
  テーブルと新しい30日レンジ情報(開始日・開始値・30日高値/安値とその日付・
  レンジ内位置%)を同じ1回のyfinance呼び出しから算出するようにした
  (API呼び出し回数は増えていない)。
- 当初は既存の通常レポートメール(`report_html`)にセクションとして追加したが、
  ユーザーの希望により**専用の別メール**に変更。`modules/stock_range.py`という
  新しいモジュールを作り、`stock_watchlist`の出力(`output/stock_watchlist.json`の
  `range_30d`)を読んで独自のHTMLメールを組み立て、`mail_gmail.send_html_mail`で
  直接送信する(通常レポートの`report_html`/`mail_gmail`パイプラインは経由しない)。
