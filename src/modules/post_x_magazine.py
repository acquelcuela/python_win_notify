...
def _send_post_notification(
    ...
    *,
    ...
    subject = f"X post {status}: {magazine.get('name') or magazine.get('id') or 'post_x_magazine'}"
...
def _build_prompt(article: dict | None, magazine: dict) -> str:
    ...
    return (
        ...
        "- 【】の中は、できるだけ具体的な数字や比較対象を入れた上で疑問形（「〜？」で終わる）にして、クリックしたくなる形にする（例:「利回り4%超の通信株、長期保有なら結局どれ？【NTT・KDDI・SB比較】」のように、数字＋比較＋疑問形を組み合わせる。内容的に疑問形が不自然な記事では無理に使わなくてよい）\n"
