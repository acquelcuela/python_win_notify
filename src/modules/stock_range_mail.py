...
def _x_trend_note(item: dict) -> str:
    finding = item.get("x_trend")
    if not finding:
        return ""
...
def run(root: Path) -> None:
    ...
    output_path = output_dir / "stock_range_mail.json"
    now = datetime.now(JST)
    generated_at = now.isoformat()

    data = _load_json(root / "output" / "stock_range.json")
    if not data or data.get("status") != "ok":
        result = {
            "module": "stock_range_mail",
            "generated_at": generated_at,
            "status": "skipped",
            "reason": "stock_range output is not available.",
        }
        output_path.write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
        logging.info("[stock_range_mail] skipped: stock_range output is not available")
        return
    ...
    subject = f"30日レンジ位置 {now.strftime('%Y-%m-%d')}"
    try:
        ...
        output_path.write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
        logging.error("[stock_range_mail] mail send failed: %s", exc)
        return
